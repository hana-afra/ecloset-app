...
class ShopDress extends ChangeNotifier {
  final List<Product> _shopdress = [];
  final List<Product> _favoriteItems = [];

  List<Product> get shopdress => _shopdress;
  ...
  Future<void> fetchDresses(BuildContext context) async {
    try {
      final isLoggedIn = await checkIfLoggedIn();
      print(isLoggedIn);
      if (isLoggedIn) {
        await fetchDressesForUser(context);
      } else {
        
        await fetchDressesForGuest();
      }
    } catch (error) {
  ...
  Future<void> fetchDressesForGuest() async {
    try {
      // Fetch products for the guest version
      var response = await dio.get(api_endpoint_dresses_get);
  ...
  Future<void> fetchDressesForUser(BuildContext context) async {
    try {
