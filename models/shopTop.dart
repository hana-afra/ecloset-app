...
class ShopTop extends ChangeNotifier {
  final List<Product> _shoptop = [];
  final List<Product> _favoriteItems = [];

  List<Product> get shoptop => _shoptop;
  ...
  Future<void> fetchTops(BuildContext context) async {
    try {
      final isLoggedIn = await checkIfLoggedIn();
      print(isLoggedIn);
      if (isLoggedIn) {
        await fetchTopsForUser(context);
      } else {
        
        await fetchTopsForGuest();
      }
    } catch (error) {
  ...
  Future<void> fetchTopsForGuest() async {
    try {
      // Fetch products for the guest version
      var response = await dio.get(api_endpoint_tops_get);
  ...
  Future<void> fetchTopsForUser(BuildContext context) async {
    try {
